        ...
        return (
            <div className="videoPage">
                <div className="controls">
                    {this.state.videos.map((d, i) => <RaisedButton onClick={() => this.chooseVideo(i)} disabled={this.state.currentVideo == i} label={d.title} className='button' key={'video-' + i}/>)}
                </div>
                <div>
                    {
											// This is a great way to conditionally show something!
	                    // Pass important information into Video through props
											selectedVideo && <Video url={selectedVideo.url} />
										}
